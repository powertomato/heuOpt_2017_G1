...
'''
    try:
        ...
        parser.add_argument("-ir", "--irace", dest="irace", action="store", type=bool, default=False, help="Irace Parameters")
        ...
        for inst in range(12, 16):
            for run in range(10):
                with open("results.txt", "a") as file:
                    inst_file = os.path.join("Instances", "instance-%02d.txt" % inst)
                    graph = Graph()
                    graph.read(os.path.join(os.getcwd(), inst_file), False)
                    print(inst_file)
                    ...
                    #plot.plot()
                    input_name = os.path.splitext(os.path.split(args.input_file[0])[1])[0]
                    irace_ext = "_irace" if irace else "_handtuned"
                    plot.save(os.path.join("GA_figs", input_name)+irace_ext+".jpg")
                    file.write('{} {} {}\n'.format(inst, run, GA.population.bestResult()))
